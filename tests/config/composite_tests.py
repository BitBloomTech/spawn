...
def test_returns_first_if_only_present_in_first(config):
    assert config.get(DEFAULT_CATEGORY, 'baz', parameter_type=int) == 42

def test_returns_second_if_only_present_in_second(config):
    assert config.get(DEFAULT_CATEGORY, 'far', parameter_type=float) == 8.3
...
def test_returns_default_if_not_present_in_either(config):
    assert config.get(DEFAULT_CATEGORY, 'workers', parameter_type=int, default=4) == 4
