...
def test_returns_int_when_int_defined_as_string():
    assert CommandLineConfiguration(foo='42').get(DEFAULT_CATEGORY, 'foo', parameter_type=int) == 42

def test_returns_float_when_int_defined_as_string():
    assert CommandLineConfiguration(foo='42').get(DEFAULT_CATEGORY, 'foo', parameter_type=float) == 42.0

def test_returns_zero_when_default_is_zero():
    assert CommandLineConfiguration(foo='0').get(DEFAULT_CATEGORY, 'foo', parameter_type=int) == 0
...
def test_returns_correct_value_when_definition_has_dot():
    assert CommandLineConfiguration(d=('foo.bar.baz=42',)).get('foo', 'bar.baz', parameter_type=int) == 42
